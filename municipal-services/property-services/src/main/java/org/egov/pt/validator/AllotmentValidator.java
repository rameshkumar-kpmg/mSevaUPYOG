...
//		System.out.println(propertyId+"----------------"+rlProps.isEmpty());
	...
	private void validateAssessees(PropertyRequest request, Property propertyFromSearch, Map<String, String> errorMap) {
		...
		Set<String> ownerMobileNumbers = propertyFromSearch.getOwners().stream().map(OwnerInfo::getMobileNo)
			...
			owners.forEach(owner -> {
				if (owner.getMobileNo() == null)
					errorMap.put("INVALID OWNER",
							" Alternate ContactNumber cannot be null for institution : " + owner.getMobileNo());
			});
		}
	...
	public void validatePropertyCriteria(PropertyCriteria criteria, RequestInfo requestInfo) {
		...
		// Commenting this down as while searching in ws for without auth its giving
		// error
		// if(propertyUtil.isPropertySearchOpen(user)) {

		// if(StringUtils.isEmpty(criteria.getLocality()))
		// throw new CustomException("EG_PT_INVALID_SEARCH"," locality is mandatory for
		// open search");
		// }
...
//					"The property mutation doesnt allow change of these fields " + fieldsUpdated);
		...
		Set<String> searchOwnerUuids = propertyFromSearch.getOwners().stream().map(OwnerInfo::getOwnerId)
				.collect(Collectors.toSet());
...
//					"Mandatory documents mising for the muation reason : " + reasonForTransfer);
	...
	public void validateAlternateMobileNumberInformation(PropertyRequest request, Property propertyFromSearch) {
		...
		for (OwnerInfo owner : property.getOwners()) {
			if (userToAlternateNumberMap.get(owner.getOwnerId()) != null
					&& userToAlternateNumberMap.get(owner.getOwnerId()).equals(owner.getMobileNo())) {
